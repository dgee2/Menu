...
### 3. Commit both the spec and the generated types

Include both `open-api/menu-api.json` and `src/generated/open-api/menu-api.ts` in the same commit as the backend change that caused the update. Do not commit them separately.
...
- Committing only the spec or only the generated types — they must stay in sync.
