...
**Prerequisite — `open-api/menu-api.json`:** `pnpm run generate-openapi` reads `../../open-api/menu-api.json` relative to `ui/menu-website/`. This file is generated by `dotnet build`. The validation sequence below runs `dotnet build` first, which will produce the file before `pnpm run generate-openapi` is called. Do not copy this file from another worktree — doing so risks using a spec that does not match the current branch.
...
**Running tests on Windows:** `pnpm run test` launches Playwright/Chromium browser tests for all 18 Storybook story files and blocks the shell until complete. On Windows agents, use PowerShell's `Start-Job` pattern (see AGENTS.md) to run with an explicit timeout and capture the exit status:
